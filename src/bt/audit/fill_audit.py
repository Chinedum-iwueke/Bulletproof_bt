...
def inspect_fill(*, ts: Any, fill: Any, bar: Any | None, tolerance_bps: float = 5.0) -> list[dict[str, Any]]:
    violations: list[dict[str, Any]] = []
    if bar is not None:
        tolerance = max(float(bar.high) * (tolerance_bps / 10_000.0), 1e-12)
        if fill.price < (bar.low - tolerance) or fill.price > (bar.high + tolerance):
            violations.append(
                {
                    "type": "fill_outside_bar",
                    "symbol": fill.symbol,
                    "ts": ts,
                    "price": fill.price,
                    "bar_low": bar.low,
