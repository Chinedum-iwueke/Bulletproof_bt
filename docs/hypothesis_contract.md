...
- For L1-H6A, `execution_semantics` additionally locks: `signal_timeframe=5m`, `base_data_frequency_expected=1m`, `strategy_family=mean_reversion`, `baseline_reference=L1-H2`, `vol_proxy=atr_over_close`, `vov_model=rolling_std_of_rv`, `gate_model=vov_quantile_gate`, `vwap_mode=session`, `stop_model=fixed_atr_multiple`, `stop_update_policy=frozen_at_entry`, `profit_exit_model=vwap_touch`, `hold_time_unit=signal_bars`, and `no_pyramiding=true`.
- For L1-H1C, `execution_semantics` additionally locks: `signal_timeframe=15m`, `base_data_frequency_expected=1m`, `strategy_family=trend_continuation`, `strategy_variant=ema_pullback`, `trend_model=ema_fast_slow`, `volatility_gate_model=natr_percentile_rank_0_100`, `adx_filter=true`, `efficiency_ratio_filter=optional_threshold`, `pullback_trigger_model=ema_fast_touch_close_back_in_trend`, `stop_model=fixed_atr_multiple_at_entry`, `stop_update_policy=frozen_at_entry`, `exit_model=ema_trend_end`, and `state_discipline=single_position_no_pyramiding`.
- For L1-H7A, `execution_semantics` additionally locks: `signal_timeframe=15m` (default; grid includes 5m), `base_data_frequency_expected=1m`, `strategy_family=trend_continuation`, `family_pattern=squeeze_expansion_pullback`, `parent_family=L1-H7`, `stop_model=fixed_atr_multiple`, `stop_update_policy=frozen_at_entry`, `tp_update_policy=tp1_then_protected_runner`, `atr_source_timeframe=signal_timeframe`, `exit_monitoring_timeframe=1m`, and `no_pyramiding=true`.
- For L1-H7B, `execution_semantics` additionally locks: `signal_timeframe=15m` (default; dedicated grid includes 5m/15m/1h), `base_data_frequency_expected=1m`, `strategy_family=trend_continuation`, `family_pattern=squeeze_expansion_pullback`, `parent_family=L1-H7`, `stop_model=fixed_atr_multiple`, `stop_update_policy=frozen_at_entry`, `tp_update_policy=tp1_then_protected_runner`, `atr_source_timeframe=signal_timeframe`, `exit_monitoring_timeframe=1m`, and `no_pyramiding=true`.
- For L1-H7C/L1-H7D/L1-H7E/L1-H7F, `execution_semantics` lock the same two-clock core (`base_data_frequency_expected=1m`, signal timeframe from the run spec on completed HTF bars only, `exit_monitoring_timeframe=1m`, `risk_accounting=engine_canonical_R`, `no_pyramiding=true`) with selected signal timeframes constrained to `{15m, 1h}`.
