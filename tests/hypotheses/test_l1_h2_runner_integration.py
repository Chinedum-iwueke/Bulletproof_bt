...
def test_runtime_override_uses_l1_h2_strategy_name() -> None:
    ...
    assert override["strategy"]["name"] == "l1_h2_compression_mean_reversion"
    assert override["strategy"]["timeframe"] == "5m"


def test_parallel_manifest_build_for_l1_h2(tmp_path: Path) -> None:
    manifest = build_hypothesis_manifest(
        hypothesis_path=Path("research/hypotheses/l1_h2_compression_mean_reversion.yaml"),
        experiment_root=tmp_path / "exp",
        phase="tier2",
    )
    assert manifest.is_file()
    assert manifest.name == "l1_h2_compression_mean_reversion_tier2_grid.csv"
