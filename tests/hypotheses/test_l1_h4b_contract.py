...
def test_l1_h4b_contract_loads_and_locks_semantics() -> None:
    ...
    assert contract.schema.metadata.hypothesis_id == "L1-H4B"
    ...
    assert sem["size_adjustment_model"] == "capped_inverse_spread_proxy_ratio"
    assert sem["spread_proxy_reference_model"] == "rolling_median"
    assert sem["baseline_reference"] == "L1-H4A"


def test_l1_h4b_parameter_grid_is_deterministic() -> None:
    contract = HypothesisContract.from_yaml("research/hypotheses/l1_h4b_liquidity_gate_size_adjusted_mean_reversion.yaml")
